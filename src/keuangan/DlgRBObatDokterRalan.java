...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
    ...
    */
    ...
    private void prosesCari() {
        try {
            // Ubah kursor menjadi kursor loading saat proses berlangsung
            this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));

            // Kosongkan tabel sebelum melakukan pengisian data
            Valid.tabelKosong(tabMode);

            // Query untuk mendapatkan daftar dokter yang memenuhi kriteria
            psdokter = koneksi.prepareStatement(
                "SELECT kd_dokter, nm_dokter FROM dokter " +
                "WHERE kd_dokter <> '-' AND status = '1' AND kd_dokter LIKE ?"
            );
            try {
                ...
                // Looping melalui hasil query dokter
                while (rsdokter.next()) {
                    tabMode.addRow(new Object[]{
                        i + ". ",
                        rsdokter.getString("nm_dokter"),
                        "",
                        "",
                        null, null, null, null, null
                    });

                    // Query untuk mendapatkan tanggal pemberian obat
                    pstanggal = koneksi.prepareStatement(
                        ...
                        "WHERE detail_pemberian_obat.status = 'Ralan' AND " +
                        "reg_periksa.kd_dokter = ? AND " +
                        "detail_pemberian_obat.tgl_perawatan BETWEEN ? AND ? " +
                        "GROUP BY detail_pemberian_obat.tgl_perawatan"
                    );
                    try {
                        pstanggal.setString(1, rsdokter.getString("kd_dokter"));
                        pstanggal.setString(2, Valid.SetTgl(Tgl1.getSelectedItem().toString()));
                        pstanggal.setString(3, Valid.SetTgl(Tgl2.getSelectedItem().toString()));
                        ...
                        while (rstanggal.next()) {
                            tabMode.addRow(new Object[]{
                                "",
                                "       " + a + ". " + rstanggal.getDate("tgl_perawatan"),
                                "",
                                "",
                                null, null, null, null, null
                            });

                            // Query pasien berdasarkan dokter dan tanggal
                            ...
                            try {
                                ...
                                while (rspasien.next()) {
                                    psobat = koneksi.prepareStatement(
                                        "SELECT detail_pemberian_obat.kode_brng, databarang.nama_brng, " +
                                    ...
                                    try {
                                        ...
                                        while (rsobat.next()) {
                                            ...
                                            subtotalHargaBeli += rsobat.getDouble("modal");

                                            tabMode.addRow(new Object[]{
                                                "", "", "",
                                                rsobat.getString("kode_brng") + " " + rsobat.getString("nama_brng"),
                                                rsobat.getDouble("jml"),
                                                rsobat.getDouble("total"),
                                                rsobat.getDouble("modal"),
                                                rsobat.getDouble("embalase"),
                                                rsobat.getDouble("tuslah")
                                            });
                                        }
                                        ...
                                        if (ttlpasienobat > 0) {
                                            tabMode.addRow(new Object[]{
                                                "", "", "Subtotal :", "",
                                                0, ttlpasienobat, subtotalHargaBeli,
                                                ttlpasienembalase, ttlpasientuslah
                                            });
                                        }
                                        ttlhargaBeli += subtotalHargaBeli; // Tambahkan subtotal harga beli ke total harga beli                                        
                                    } finally {
                                ...
                                a++;
                                // Tambahkan total keseluruhan ke tabel
                                        tabMode.addRow(new Object[]{
                                            ">>", "Total :", "",
                                            "", null, ttlbiaya, ttlhargaBeli, ttlembalase, ttltuslah
                                        });
                            } finally {
